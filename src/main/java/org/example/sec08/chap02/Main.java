package org.example.sec08.chap02;

import org.example.sec07.chap04.*;
...
public class Main {
    public static void main(String[] args) {
        ...
        // Object를 반환하기 때문에 명시적 형변환이 필요하다.
        ArrayList<Integer> ints4 = (ArrayList<Integer>) ints3.clone();



        ints3.remove(4); // int: 인덱스로 지우기
        // 파라미터값이 정수일 경우에 index 값인지 요소인지 구별하기 위해
        // Wrapper 클래스로 명시적으로 변환해준다.
...
//        knights.add(new Swordman(Side.BLUE)); // ⚠️ 불가 (부모요소이기 때문)
...
//        eliteSquad = new ArrayList<Swordman>(); // ⚠️ 불가
        ...
        ArrayList<? super Knight> eliteSquad2;

        eliteSquad2 = new ArrayList<Swordman>();
        eliteSquad2 = new ArrayList<Knight>();
//        eliteSquad2 = new ArrayList<MagicKnight>(); // ⚠️ 불가
        ...
        //  요소가 하나 지워졌는지 여부 반환
        boolean removed1 = knights.remove(new Knight(Side.RED)); // 참조값이 다르기 때문에 삭제안됨
        boolean removed2 = knights.remove(knight1); // 삭제 ⭕

        // ✅ 이 위에 있는 ArrayList 들을 전부 LinkedList 로 바꿔도 적용됨
        ...
        // push 메소드의 내부를 보면 addFirst 를 사용
        charLList.push('A');
        charLList.push('B');
        charLList.push('C');
        charLList.push('D');
        ...
        // pop 메소드의 내부를 보면 removeFirst 를 사용
